#include "../../../../dycoon_labo/experimentals/cor_matrix/cpp/cor_matrix/binding_interface/matrix.h"
#include "../../dycoon_labo/experimentals/cor_matrix/cpp/cor_matrix/generated_binding.h"

namespace cor
{
    namespace cor_matrix_cpp_gen_binding
    {
        
          void GeneratedBinding_bind_func_0(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_1(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_2(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_3(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_4(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_5(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_6(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_7(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_8(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_9(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_10(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_11(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_12(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_13(mruby_interface::MrubyState& mrb);
          void GeneratedBinding_bind_func_14(mruby_interface::MrubyState& mrb);

        
        bool GeneratedBinding_RCharArray_valid_question(std::weak_ptr<cor::RCharArray> c);
        std::weak_ptr<cor::RCharArray> GeneratedBinding_RCharArray_create_0();
        std::weak_ptr<cor::RCharArray> GeneratedBinding_RCharArray_create_1(std::allocator<char> a0);
        cor::matrix::Matrix GeneratedBinding_cor__matrix__Matrix_create_dense_by_size(int a0, int a1);
        cor::matrix::Matrix GeneratedBinding_cor__matrix__Matrix_create_dense_by_array(mrubybind::MrubyRef a0);
        double GeneratedBinding_cor__matrix__Matrix_get(cor::matrix::Matrix& c, int a0, int a1);
        void GeneratedBinding_cor__matrix__Matrix_set(cor::matrix::Matrix& c, int a0, int a1, double a2);
        void GeneratedBinding_cor__RCharArray_capacity(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_begin_1(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_begin_2(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_end_1(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_end_2(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_rbegin_1(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_rbegin_2(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_rend_1(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_rend_2(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_cbegin(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_cend(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_crbegin(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_crend(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_shrink_to_fit(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_size(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_max_size(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_empty(std::weak_ptr<cor::RCharArray> c);
        std::allocator<char> GeneratedBinding_cor__RCharArray_get_allocator(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_data_1(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_data_2(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_front_1(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_front_2(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_back_1(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_back_2(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_pop_back(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__RCharArray_clear(std::weak_ptr<cor::RCharArray> c);
        void GeneratedBinding_cor__mruby_interface__AnyWP_reset(cor::mruby_interface::AnyWP& c);
        void GeneratedBinding_cor__mruby_interface__AnyWP_expired(cor::mruby_interface::AnyWP& c);
        void GeneratedBinding_cor__mruby_interface__AnyWP_lock(cor::mruby_interface::AnyWP& c);

    }
}

